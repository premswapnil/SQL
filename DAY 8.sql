...
-- This table treated as parent table
create Table P1(
...
--Step 2 --> Will insert the values
insert P1 values
(10,'SQL','Blr'),(20,'SAP','Hyd'),(30, 'Account','Mumbai')

select * from P1

--- Step3--> Will create another table
---- This table consider as child table
---- Here you need specify on which table you need provide relation

create table C1(
Empid int,
Ename varchar(255),
salary int,
Deptno int foreign key references P1(Deptno)
)

-- you need to identify weather there is a relation exist between the tables or not
-- so here c1 table can access Deptno values of P1 table i.e 10,20,30
-- if you try to insert other than deptno values of p1 table, it will not allow
...
--- PK, FK we will look into 3 cases / 3 rules
--- a> Insertion Rule
--- b> Updation Rule
--- c> Deletion Rule

--- a> Insertion Rule
--- if you try to insert anew value in child table(FK), Which is not there in parent table(PK)
--- it will not allow
--- Insertion not allowed in child table, But you can insert new values in parent table
...
--- b> Updation Rule
--- if you want update any value in parnts table will not allow(pk)
...
--- c> Deletion Rule
--- suppose if you want delete a parent table data
delete P1
where Deptno=10

-- insertion allows in parent table
-- updation not allows in parents table
-- deletion not allows in parent table

--- Insertion rule is fixed
--- parent table does not have ---> but child table have

--- In Update Cascade
--- This rule will ise when ever we want update the data in parent along with child table

--- On Delete Cascade
--- This rule will use whenever we want delete the data in parent table along with child table
...
insert P2 values
(10,'SQL','Blr'), (20,'SAP','Hyd'),(30,'Accounts','Mumbai')
...
Ename varchar(255),
Salary int,
Deptno int foreign key references P2(Deptno)
on update cascade
on delete cascade
)
insert C2 values
